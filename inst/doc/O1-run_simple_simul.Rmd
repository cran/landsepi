...
Buit-in parameterisation for different pathogens (e.g. rusts of cereal crops) is available using function `loadPathogen()`:
```{r}
...
$$IR(t) = IR_{max} \times  \left(1 - \frac{treatment\_efficiency}{1+e^{3.5- 9.0 \times C(t)}}\right)$$
...
Cultivars to be treated with chemicals and dates of applications are defined
with parameters `treatment_cultivars` and `treatment_timesteps`, and are the same for all polygons 
cultivated with the cultivars to be treated. The cost of a single treatment application (monetary units/ha) 
is defiend by `treatment_cost` and will impact the economic outputs.
Treatment parameters can be set via the function `setTreatment`:
