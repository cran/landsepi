...
runShinyApp <- function() {
  ...
  needed_packages <- c("shiny", "shinyBS", "shinyalert", "DT", "shinyjs", "gridExtra", "png", "tiff", "grid", "future", "promises", "tools")
  if( sum(needed_packages %in% utils::installed.packages()[,1] == FALSE) != 0) {
    stop('Install packages : install.packages(c("shiny","shinyBS", "shinyalert", "DT", "shinyjs", "gridExtra", "png", "tiff", "grid", "future", "promises", "tools"))')
  } 
